// This is  CSCHitFromComparatorOnly.cc

#include "RecoLocalMuon/CSCRecHitD/src/CSCHitFromComparatorOnly.h"
...
CSCHitFromComparatorOnly::CSCHitFromComparatorOnly( const edm::ParameterSet& ps ) : recoConditions_(0), /*calcped_(0),*/ ganged_(0) {
  ...
  //LogTrace("CSCRecHit")  << "[CSCHitFromComparatorOnly] CSCUseStaticPedestals = " << useStaticPedestals;
  //if ( !useStaticPedestals ) LogTrace("CSCRecHit")  << "[CSCHitFromComparatorOnly] CSCNoOfTimeBinsForDynamicPedestal = " 
...
CSCHitFromComparatorOnly::~CSCHitFromComparatorOnly() {
  //delete calcped_;
}
 ...
 */
std::vector<CSCStripHit> CSCHitFromComparatorOnly::runCompStrip( const CSCDetId& id, const CSCLayer* layer,
								 const CSCComparatorDigiCollection::Range& rcompd ) 
{	
  ...
  LogTrace("CSCHitFromComparatorOnly")  << "[CSCHitFromComparatorOnly::runStrip] id= " << id_ << 
 ...
 *
 */
float CSCHitFromComparatorOnly::makeCluster( int centerStrip ) {
  ...
  LogTrace("CSCHitFromComparatorOnly")  << "[CSCHitFromComparatorOnly::makeCluster] centerStrip= " << centerStrip << 
    " strippos=" << strippos;
 ...
 *
 */
CSCStripHitData CSCHitFromComparatorOnly::makeStripData(int centerStrip, int offset) {
  ...
  int istop  = std::min( tmax+2, 13 ) ; // there are only time bins 0-13
  // adc[3] = 0.1; // in case it isn't filled
  adc[3] = 0.; // in case it isn't filled
	
  //if ( tmax > 2 && tmax < 7 ) { // for time bins 3-6
  if ( tmax > 2 && tmax < 13 ) { // for time bins 3-12
    ...
    adc[3] = 0.;
    adcRaw = adc;
    LogTrace("CSCRecHit")  << "[CSCHitFromComparatorOnly::makeStripData] Tmax out of range: contact CSC expert!";
  }
  ...
  if ( offset == 0 ) {
	...
	adc1[3]    = 0.; 
        ...
        if ( tmax > 2 && tmax < 13 ) { // for time bin tmax from 3-14
 ...
 *
 */
//void CSCHitFromComparatorOnly::fillPulseHeights( const CSCStripDigiCollection::Range& rstripd ) {
void CSCHitFromComparatorOnly::fillPulseHeights( const CSCComparatorDigiCollection::Range& rcompd ) {
  ...
  for ( CSCComparatorDigiCollection::const_iterator it = rcompd.first; it != rcompd.second; ++it ) {
    ...
    auto const &  scaOritmp = (*it).getTimeBinsOn();
    //std::vector<int> scaOri(8, 0); 
    std::vector<int> scaOri(14, 0); 
    for(size_t iv=0; iv<scaOritmp.size(); ++iv) { 
      //if(scaOritmp[iv]<8) scaOri[scaOritmp[iv]] = 1; 
      if(scaOritmp[iv]<14) scaOri[scaOritmp[iv]] = 1; 
    } 
    //assert(scaOri.size()==8);
    assert(scaOri.size()==14);
    ...
    //@@ Find bin with largest pulseheight (in fact, all maxima are 1, so just find the first) 
 ...
 */
void CSCHitFromComparatorOnly::findMaxima(const CSCDetId& id) {
  ...
  for ( size_t i=0; i!=thePulseHeightMap.size(); ++i ) {
    ...
    if(maximumFound){
      ...
      float testThreshold = 0.5; //---- half comparator output 
...
bool CSCHitFromComparatorOnly::isPeakOK(int iStrip, float heightCluster){
  int i = iStrip;
  bool peakOK =  ( thePulseHeightMap[i].phmax()>theThresholdForAPeak &&  
		   ...
		   thePulseHeightMap[i].tmax() > 2 && thePulseHeightMap[i].tmax() < 13); 
  return peakOK;
}
 ...
 *
 */
float CSCHitFromComparatorOnly::findHitOnStripPosition( const std::vector<CSCStripHitData>& data, const int& centerStrip ) {
  ...
  for ( size_t i = 0; i != data.size(); ++i ) {
    ...
    */
    ...
    if ( data[i].strip() < 1 ){
      LogTrace("CSCRecHit") << "[CSCHitFromComparatorOnly::findHitOnStripPosition] problem in indexing of strip, strip= " 
        << data[i].strip();
    } 
    sum_w += w[1];
...
bool CSCHitFromComparatorOnly::isNearDeadStrip(const CSCDetId& id, int centralStrip, int  nstrips){
...
bool CSCHitFromComparatorOnly::isDeadStrip(const CSCDetId& id, int centralStrip, int  nstrips ){
  return recoConditions_->badStrip( id, centralStrip, nstrips );

} 

// Define space for static
const int CSCHitFromComparatorOnly::theClusterSize;
