// This is CSCRecHitFromComparatorBuilder.cc
...
CSCRecHitFromComparatorBuilder::CSCRecHitFromComparatorBuilder( const edm::ParameterSet& ps ) : geom_(0) {
  ...
  hitsFromComparatorOnly_= new CSCHitFromComparatorOnly( ps ); 
...
CSCRecHitFromComparatorBuilder::~CSCRecHitFromComparatorBuilder() {
  delete hitsFromComparatorOnly_;
...
void CSCRecHitFromComparatorBuilder::build( const CSCComparatorDigiCollection* compdigis, const CSCWireDigiCollection* wiredc,
                               CSCRecHit2DCollection& oc ) {
  LogTrace("CSCRecHit") << "[CSCRecHitFromComparatorBuilder] build entered";

  if ( !geom_ ) throw cms::Exception("MissingGeometry") << "[CSCRecHitFromComparatorBuilder::getLayer] Missing geometry" << std::endl;
  ...
  for ( CSCComparatorDigiCollection::DigiRangeIterator it=compdigis->begin(); it!=compdigis->end(); ++it) { 
    const CSCDetId& id = (*it).first;
    const CSCLayer* layer = getLayer( id );
    const CSCComparatorDigiCollection::Range& rcompd = (*it).second;
    ...
    std::vector<CSCStripHit> const & cscStripHit = hitsFromComparatorOnly_->runCompStrip(id, layer, rcompd);
    ...
    LogTrace("CSCRecHitBuilder")<< "[CSCRecHitFromComparatorBuilder] found " << cscStripHit.size() << " strip and " 
    ...
    LogTrace("CSCRecHit") << "[CSCRecHitFromComparatorBuilder] " << hits_in_layer << " rechits found in layer " << sDetId;
    // std::cout << "[CSCRecHitFromComparatorBuilder] " << hits_in_layer << " rechits found in layer " << sDetId << std::endl;
  ...
  LogTrace("CSCRecHit") << "[CSCRecHitFromComparatorBuilder] " << oc.size() << " 2d rechits created in this event.";

}


const CSCLayer* CSCRecHitFromComparatorBuilder::getLayer( const CSCDetId& detId )  {
  return geom_->layer(detId);
}


void CSCRecHitFromComparatorBuilder::setConditions( CSCRecoConditions* reco ) {
  recoConditions_ = reco;
  hitsFromComparatorOnly_->setConditions( reco );
