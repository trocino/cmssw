#ifndef CSCRecHitD_CSCHitFromComparatorOnly_h
#define CSCRecHitD_CSCHitFromComparatorOnly_h

/** \class CSCHitFromComparatorOnly (old comment below)
 *
...
class CSCHitFromComparatorOnly 
{
 ...
 public:
  ...
  explicit CSCHitFromComparatorOnly( const edm::ParameterSet& ps );
  
  ~CSCHitFromComparatorOnly();
  
  std::vector<CSCStripHit> runCompStrip( const CSCDetId& id, const CSCLayer* layer, const CSCComparatorDigiCollection::Range& rcompd );
 ...
 private:
  ...
  void fillPulseHeights( const CSCComparatorDigiCollection::Range& rcompd );  
...
// MEMBER DATA
  ...
  //float gainWeight[80]; // useless, but needed for recoConditions_->stripWeights(...)
